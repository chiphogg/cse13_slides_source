...
low.noise <- 1e-6
X.data <- HexagonalGrid(n=5, unit=1)
m.true <- Model("true")
data.0 <- data.frame(X=X.data, Y=X.data[, 1])
d.0 <- Dataset(id='throwaway', data=data.0,
    X.names=c('X.1', 'X.2'), column='Y')
```

`r print(head(data.0))` is so great

---

```{r test3d_more_setup, echo=FALSE, include=FALSE}
Y.true <- m.true$L(d=d.0) %*% rnorm(n=d.0$n)
m.true$AddCovariance(CovarianceSE(ell=6, sigma.f=1))
data <- Dataset(id='random',
  data=data.frame(X=X.data, Y=Y.true + 0.1 * rnorm(nrow(X.data))),
  X.names=c('X.1', 'X.2'), column='Y')
X.plot <- HexagonalGrid(n=30, unit=0.1)
N.plot <- nrow(X.plot)

# Create gppois models to train on datapoints
m.wide <- Model(id="wide")
m.wide$AddCovariance(CovarianceSE(ell.range=c(5, 10), sigma.f.range=c(0.01, 1000)))
m.wide$SetNoiseBounds(low.noise)
m.jumpy <- Model(id="jumpy")
m.jumpy$AddCovariance(CovarianceSE(ell.range=c(0.2, 1), sigma.f.range=c(0.01, 1000)))
m.jumpy$SetNoiseBounds(low.noise)
#m.wide$Train(d=data)
