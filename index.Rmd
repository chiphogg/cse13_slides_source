...
knit_hooks$set(webgl = function(before, options, envir) {
  ...
  Sys.sleep(.25) # need time to respond to window size change
...
```{r wxyz, webgl=TRUE, fig.width=8, fig.height=9}
x <- sort(rnorm(1000))
y <- rnorm(1000)
...
```{r test3d_setup, webgl=TRUE, echo=FALSE, include=FALSE}
require(rgl)
require(gppois)
set.seed(2)

# Generate synthetic datapoints and plotting grid
X.data <- HexagonalGrid(n=3, unit=1)
data <- Dataset(id='random',
  data=data.frame(X=X.data, Y=0.5 * rnorm(nrow(X.data))),
...
m.wide$AddCovariance(CovarianceSE(ell=5, sigma.f=0.1))
m.wide$SetNoiseBounds(low.noise)
m.jumpy <- Model(id="jumpy")
m.jumpy$AddCovariance(CovarianceSE(ell=0.5, sigma.f=1))
